...
用户可以还原设置并退回到手机桌面，点击顶部功能栏的**运行**按钮，验证脚本能否正常执行。  

### 3）示例3: IDE中编写iOS设备脚本
以上两个示例都是基于Android设备的，WeAutomator IDE支持连接iOS设备，我们可以使用
IDE连接iOS设备，并编写脚本。  
示例3将通过IDE编写iOS脚本：打开iOS设备的safari，搜索特定视频网址，并播放视频。使用者可以
根据本示例了解如何使用IDE编写针对iOS设备的脚本。  
- 打开safari  
和Android设备中的启动app一样，也是先打开iOS设备的safari，进入safari界面后，点击右部功能栏，
点击**启动APP**，在跳出的会话框中点击**从当前页面获取**，最后点击**确定**，完成启动iOS设备safari的代码生成。  
![figure](image/iOS-start-safari.png)  

- 输入网址 
打开录制功能，选择**原生**，等控件树界面打开后，选择网址搜索框并点击，IDE将生成点击
safari网址搜索框的代码， 接着点击右部功能框的**输入文本**，在跳出的会话框中，将目标网址
复制进去，并点击确定，IDE便生成输入网址的代码。  
![figure](image/safari-search-web.png)  
- 前往目标网址  
使用快捷键，生成基于图像的点击操作，点击当前页面上的**前往**按钮，跳转到目标网址。  
- 播放视频  
生成基于位置坐标的点击代码，点击视频区域，实现播放。  
